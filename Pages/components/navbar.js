function navbar(){
    return `
        <div id="top">
            <div id="top1">
            <p>PB Contract</p>
            <p>PB/Apartment</p>
            <p>Pottery Barn Kids</p>
            <p>Pottery Barn Teen</p>
            <p>Williams Sonoma</p>
            <p>WS Home</p>
            <p>West Elm</p>
            <p>Rejuvenation</p>
            <p>Mark & Graham</p>
        </div>
            <div id="top2">
                <p>My Store</p>
                <p>Key Rewards</p>
                <p>Track Order</p>
                <p>Ship To</p>
            </div>
        </div>
        <div id="grey-headbar"></div>
        <div id="navbar1">
            <div class="navbar-1"> <input type="text" id="search-field" name="words" placeholder="Search" value="" maxlength="100" autocorrect="off" autocomplete="off"/><span><i class="fa fa-search" style="font-size:15px;padding-top: 15px;margin-right: 30px;color: gray;"></i></span></div>
           
            <div class="navbar-1" style="padding-bottom: 50px;"><img  style="width: 100%; height: 28px; color: black;" src="https://www.potterybarn.com/netstorage/images/logo_pb.gif" alt=""></div>
            <!-- <p>POTTERYBARN</p> -->
            <div class="navbar-1">
                <div>
                    <i class="fa fa-user" style="font-size:15px;padding-left: 20px;"></i></li>
                    <li style="font-size: smaller;">My Account</li>
                </div>
                <div>
                    <i class="fa fa-envelope" style="font-size:15px;padding-left: 35px;"></i></li>
                    <li style="font-size: smaller;">Wedding Registry</li>
                </div>
                <div>
                    <i class="material-icons" style="font-size:15px;padding-left: 18px;">favorite</i></li>
                    <li style="font-size: smaller;">Favorites</li>
                </div>
                <div>
                    <i class="fa fa-shopping-cart" style="font-size:15px;padding-left: 20px;"></i></li>
                    <li style="font-size: smaller; padding-left: 15px;">Cart</li>
                </div>
            </div>
        </div>
       
        <div class="wrapper">
        <div id="sec-nav" class="mega_menu">
            
            <li>New &amp; Now</li>
            <div class="sub_menu">
                <div class="col">
                    <img src="https://assets.pbimgs.com/pbimgs/rk/images/dp/ecm/202152/1648/001/003.jpg" alt="">
                    <p>The Spring Lookbook ›</p>
                    
                </div>
                <div class="col">
                    <img src="https://assets.pbimgs.com/pbimgs/rk/images/dp/ecm/202213/1831/001/001.jpg" alt="">
                    <p>New Arrivals › </p>
                </div>
                <div class="col">
                    <img src="https://assets.pbimgs.com/pbimgs/rk/images/dp/ecm/202213/0551/001/001.jpg" alt="">
                    <p>The Outdoor Lookbook ›</p>
                </div>
                <div class="col2">
                <div> 
                    <h5>Lookbooks</h5>
                    <p>The Spring Lookbook</p>
                    <p>The Outdoor Lookbook</p>
                    <h5>New Arrivals</h5>
                    <p>Shop All</p>
                    <p>Furniture</p>
                    <p>Outdoor & Garden</p>
                    <p>Bedding</p>
                    <p>Bath</p>
                    <p>Lighting</p>
                    <p>Rugs</p>
                </div>
    
    
                <div><h5>Trending Now</h5>
                    <p>Misty Morning Palette</p>
                    <p>Textured Solids</p>
                    <p>Recycled & Colored Glass</p>
                    <p>Woven Textures & Rattan</p>
                    <p>Artisanal Ceramics</p>
                </div>
                <div>
                    <h5>Room Inspiration</h5>
                    <p>Living Rooms</p>
                    <p>Bedrooms</p>
                    <p>Dining Rooms</p>
                    <p>Home Offices</p>
                    <p>Bathrooms</p>
                    <p>Entryways</p>
                    <p>Kitchens</p>
                    <p>Small Spaces</p>
                    <p>Outdoor</p>
                    <p>Shop All</p>
    
                </div>
            </div>
            </div>
            <li>In-Stock</li>
            <li>Bestseller</li>
            <li>Collaborations</li>
            <li>Design Services</li>
            <li>Sustainibility</li>
       
        </div>
    </div>
        <div id="third-nav" class="mega_menu2">
            <div>Furniture</div>
           
                <div class="sub_menu2">
                   
                    <div class="col3"> 
                        <h5>Lookbooks</h5>
                        <p>The Spring Lookbook</p>
                        <p>The Outdoor Lookbook</p>
                        <h5>New Arrivals</h5>
                        <p>Shop All</p>
                        <p>Furniture</p>
                        <p>Outdoor & Garden</p>
                        <p>Bedding</p>
                        <p>Bath</p>
                        <p>Lighting</p>
                        <p>Rugs</p>
                    </div>
        
        
                    <div class="col3">
                         <h5>Trending Now</h5>
                        <p>Misty Morning Palette</p>
                        <p>Textured Solids</p>
                        <p>Recycled & Colored Glass</p>
                        <p>Woven Textures & Rattan</p>
                        <p>Artisanal Ceramics</p>
                    </div>
                    <div class="col3">
                        <h5>Room Inspiration</h5>
                        <p>Living Rooms</p>
                        <p>Bedrooms</p>
                        <p>Dining Rooms</p>
                        <p>Home Offices</p>
                        <p>Bathrooms</p>
                        <p>Entryways</p>
                        <p>Kitchens</p>
                        <p>Small Spaces</p>
                        <p>Outdoor</p>
                        <p>Shop All</p>
        
                  
                </div>
                </div>
    
                
            <div><span>Outdoor & Garden</span></div>
            <div>Bedding</div>
            <div>Bath</div>
            <div>Lighting</div>
            <div>Rugs</div>
            <div>Windows</div>
            <div>Pillows & Decor</div>
            <div>Mirrors & Arts</div>
            <div>Tabletop & Bar</div>
            <div>Storage</div>
            <div>Ester</div>
            <div>Gifts</div>
            <div>Sale</div>
        </div>
        <div id="forth-nav">
            <div class="forth-nav-class"><h6>ENDS TOMORROW</h6>
            <P>SPRING WAREHOUSE EVENT</P></div>
            <div class="forth-nav-class" style="letter-spacing: 1px;"><h6>Up to 60% off Furniture, Bedding, Decor & More</h6>
               <button id="btn">SHOP NOW</button></div>`

}
export default navbar;